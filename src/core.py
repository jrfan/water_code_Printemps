...
class WaterManagementSimulation:
    ...
    def run_simulation(self, seed: int = None) -> None:
        ...
        for iteration in range(self.nb_iterations):
            ...
            for turn in range(self.total_turns):
                ...
                #     print("=== Final Turn Actor Summary ===")
                #     for i in range(self.nb_actors):
                #         total_reward = np.sum(self.h_rewards[i, self.it, :])
                #         total_fine = np.sum(self.h_fines[i, self.it, :])
                #         total_subsidy = np.sum(self.h_subventions[i, self.it, :])
                #         total_incentive = np.sum(self.h_policies[i, self.it, :])
                #         total_pumped = np.sum(self.h_water_pump[i, self.it, :])
                #         total_used = np.sum(self.h_water_used[i, self.it, :])
                #         print(f"Actor {i}:")
                #         print(f"  Priority:        {self.actors_priority[i]}")
                #         print(f"  Demand:          {self.actors_demands[i]:.2f}")
                #         print(f"  Total Reward:    {total_reward:.2f}")
                #         print(f"  Total Fine:      {total_fine:.2f}")
                #         print(f"  Total Subsidy:   {total_subsidy:.2f}")
                #         print(f"  Total Incentive: {total_incentive:.2f}")
                #         print(f"  Total Pumped:    {total_pumped:.2f}")
                #         print(f"  Total Used:      {total_used:.2f}")
                #         print(f"  Final Quota:     {self.h_quota[i, self.it, self.trn]:.2f}")
                #         print(f"  Final Pumped:    {self.h_water_pump[i, self.it, self.trn]:.2f}")
                #         print(f"  Final Incentive: {self.h_policies[i, self.it, self.trn]:.2f}")
                #         print(f"Total Econ Impact: {np.sum(self.h_econ_impacts[i, self.it, :]):.2f}")
                #         total_cooperation = np.sum(self.h_actions[i, self.it, :] == 1)
                #         print(f"  Cooperation Count: {total_cooperation} / {self.total_turns}")
    def _process_turn(self) -> None:
        ...
        self.w_riverflows[self.it, self.trn] = self.ecology_manager.calculate_riverflow(
            self.trn
        )
        ...
        diff_rewards = self.actor_manager.compute_alternative_rewards(
            riverflow_predictions
        )
        self.h_diff_rewards[:, self.it, self.trn] = diff_rewards
        self.h_alt_storage[:, self.it, self.trn] = (
            self.actor_manager.get_previous_storage()
        )
